...
const supabaseClient = SUPABASE_URL.startsWith("http") ? window.supabase.createClient(SUPABASE_URL, SUPABASE_ANON_KEY) : null;
...
function livePreview() {
  ...
  document.getElementById("previewMsg").classList.toggle("is-lyrics", sendMode === "lyrics");
  const ps = document.getElementById("previewSong");
  if (selectedSong) {
    ps.style.display = "flex";
    ps.innerHTML = `<img class="preview-song-art" src="${selectedSong.art}" alt=""/> ${sendMode === "lyrics" ? "📜 Lyrics from" : "♫"} ${selectedSong.title} — ${selectedSong.artist}`;
  } else {
...
function setSendMode(mode) {
  ...
  const ta = document.getElementById("msgField");
  ta.placeholder = mode === "lyrics" ? "Pick lines from the song below, or type your own take on them…" : "Write something honest, kind, brave — or all three…";
  livePreview();
}
...
async function sendMsg() {
  ...
  const btn = document.getElementById("sendBtn");
  if (!to) {
    showBtnErr(btn, "Who is this for?");
    return;
  }
  if (!msg) {
    showBtnErr(btn, "Write something first");
    return;
  }
  if (!supabaseClient) {
    showBtnErr(btn, "Database not configured");
    return;
  }
  ...
  btn.disabled = true;
  const {data: data, error: error} = await supabaseClient.from("messages").insert({
    to_name: to,
    message: msg,
    style: selectedStyle,
    song: song,
...
function showBtnErr(btn, text) {
  ...
  setTimeout(() => {
    ...
    btn.querySelector(".send-inner").innerHTML = `<svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg> Send Anonymously`;
  }, 2200);
}
...
function showSongResults(tracks) {
  ...
  el.innerHTML = tracks.map((t, i) => {
    ...
    const mins = Math.floor(t.trackTimeMillis / 6e4);
    const secs = String(Math.floor(t.trackTimeMillis % 6e4 / 1e3)).padStart(2, "0");
...
function toggleLyricLine(idx) {
  lyricLines[idx].checked = !lyricLines[idx].checked;
  renderLyricsChecklist();
}

function updateLyricsFooter() {
  const n = lyricLines.filter(l => l.checked).length;
  const btn = document.getElementById("lyricsUseBtn");
  if (!btn) return;
  btn.disabled = n === 0;
  btn.textContent = n > 0 ? `Use ${n} line${n > 1 ? "s" : ""} ✦` : "Select at least one line";
}
...
document.addEventListener("click", e => {
  if (!e.target.closest(".song-search-wrap")) document.getElementById("songResults").classList.remove("open");
});
...
function renderMessages(filter = "") {
  ...
  const filtered = filter ? inbox.filter(m => m.to.toLowerCase().includes(filter)) : inbox;
  ...
  if (!item.opened) {
    ...
    if (supabaseClient) {
      supabaseClient.from("messages").update({
        opened: true
      }).eq("id", item.id).then(({error: error}) => {
...
function fitLyricText(phrase) {
  const text = document.getElementById("lyricPopText");
  const len = phrase.length;
  let size;
  if (len <= 18) size = 36; else if (len <= 32) size = 30; else if (len <= 48) size = 25; else if (len <= 68) size = 21; else if (len <= 95) size = 17; else size = 14.5;
...
function playLyricPop(message, timings, onDone) {
  ...
  const nextPhrase = () => {
    ...
    const nextWord = () => {
      ...
      span.textContent = (wIdx > 0 ? " " : "") + words[wIdx];
...
function stopLyricPop() {
  clearTimeout(lyricPopTimer);
  const stage = document.getElementById("lyricPopStage");
  const text = document.getElementById("lyricPopText");
  stage.classList.remove("active");
  text.innerHTML = "";
...
function revealAnimBottle(onDone) {
  revealAnimBase((ctx, W, H, p, t) => {
    ...
    if (t % 4 === 0) for (let i = 0; i < 6; i++) {
      ...
      ctx.beginPath();
      ctx.arc(gx, gy, rand(.5, 2), 0, Math.PI * 2);
      ctx.fillStyle = `rgba(125,211,252,${rand(.3, .8)})`;
      ctx.fill();
    }
    const bottleY = lerp(H + 80, H * .35, ease(p));
    ctx.save();
    ctx.translate(W * .5, bottleY);
...
function revealAnimLetter(onDone) {
  ...
  revealAnimBase((ctx, W, H, p, t) => {
    ctx.fillStyle = `rgba(10,5,30,${p * .88})`;
    ctx.fillRect(0, 0, W, H);
    for (let i = 0; i < 60; i++) {
      const a = .3 + Math.sin(t * .04 + i) * .2;
      ctx.beginPath();
      ctx.arc(i * 137.5 % W, i * 89.3 % (H * .6), .8, 0, Math.PI * 2);
    ...
    if (p < .6) for (let l = 1; l <= 4; l++) {
      ctx.beginPath();
      ctx.moveTo(ex - l * 28, ey - l * 7);
      ctx.lineTo(ex - l * 28 + 22, ey - l * 7);
      ctx.strokeStyle = `rgba(165,180,252,${.25 - l * .05})`;
      ctx.lineWidth = 2;
      ctx.stroke();
    }
    if (t >= 60) {
      sparks.forEach(s => {
        const age = t - s.born;
        if (age < 0 || age > s.maxLife) return;
        const lf = age / s.maxLife;
        ctx.beginPath();
        ctx.arc(W * .5 + s.vx * age, H * .38 + s.vy * age + .03 * age * age, 2 * (1 - lf), 0, Math.PI * 2);
        ctx.fillStyle = `rgba(196,181,253,${1 - lf})`;
        ctx.fill();
      });
    }
    ctx.save();
...
function revealAnimDove(onDone) {
  revealAnimBase((ctx, W, H, p, t) => {
    ...
    [ [ .2, .15 ], [ .7, .1 ], [ .5, .22 ] ].forEach(([cx, cy], i) => {
      ctx.save();
      ctx.translate(W * cx + Math.sin(t * .01 + i) * 10, H * cy);
      ctx.scale(1 + i * .4, 1);
      ctx.globalAlpha = p * .3;
      ctx.beginPath();
...
function revealAnimStar(onDone) {
  ...
  revealAnimBase((ctx, W, H, p, t) => {
    ctx.fillStyle = `rgba(4,2,20,${p * .95})`;
    ctx.fillRect(0, 0, W, H);
    stars.forEach(s => {
      const a = .4 + Math.sin(t * .05 + s.twinkle) * .3;
      ctx.beginPath();
      ctx.arc(s.x * W, s.y * H, s.r, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(255,255,255,${a * p})`;
      ctx.fill();
    });
    ...
    if (burstP > 0) {
      ...
      for (let i = 0; i < 16; i++) {
        ...
        ctx.strokeStyle = `rgba(253,224,71,${(1 - burstP) * .8})`;
        ctx.lineWidth = 2;
        ctx.stroke();
      }
      const g = ctx.createRadialGradient(cx, cy, 0, cx, cy, 60 * burstP);
      g.addColorStop(0, "rgba(254,240,138,.85)");
      g.addColorStop(1, "transparent");
      ctx.beginPath();
      ctx.arc(cx, cy, 60 * burstP, 0, Math.PI * 2);
      ctx.fillStyle = g;
      ctx.fill();
    }
  }, 95, onDone);
...
function revealAnimLantern(onDone) {
  ...
  revealAnimBase((ctx, W, H, p, t) => {
    ...
    for (let i = 0; i < 80; i++) {
      ...
      ctx.beginPath();
      ctx.arc(i * 137.5 % W, i * 89.3 % (H * .7), .7, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(255,255,200,${a * p})`;
      ctx.fill();
    }
    motes.forEach(m => {
      m.life += .012;
      if (m.life > 1) m.life = 0;
      const a = Math.sin(m.life * Math.PI) * p;
      ctx.beginPath();
      ctx.arc(m.x * W, H - m.life * H * .6, m.size, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(251,191,36,${a * .6})`;
      ctx.fill();
    });
    const ly = lerp(-80, H * .35, ease(p));
    const glow = .5 + Math.sin(t * .1) * .3;
    const halo = ctx.createRadialGradient(W * .5, ly, 0, W * .5, ly, 120);
    halo.addColorStop(0, `rgba(251,146,60,${glow * .4 * p})`);
    halo.addColorStop(1, "transparent");
    ctx.beginPath();
    ctx.arc(W * .5, ly, 120, 0, Math.PI * 2);
    ctx.fillStyle = halo;
    ctx.fill();
    ctx.save();
    ctx.translate(W * .5, ly);
    ctx.scale(5, 5);
    ctx.rotate(Math.sin(t * .03) * .1);
...
function revealAnimScroll(onDone) {
  revealAnimBase((ctx, W, H, p, t) => {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      ctx.beginPath();
      ctx.arc(mx, my, rand(1, 3), 0, Math.PI * 2);
      ctx.fillStyle = `rgba(202,138,4,${a * p})`;
      ctx.fill();
    }
    ...
    ctx.strokeStyle = `rgba(202,138,4,${p * .7})`;
    ctx.lineWidth = 2;
    rrect(ctx, sx, sy, sw, openH, 5);
    ctx.fill();
    ctx.stroke();
    for (let l = 0; l < 8; l++) {
      const ly = sy + 20 + l * 30;
      if (ly > sy + openH - 18) break;
      ctx.beginPath();
      ctx.moveTo(sx + 16, ly);
      ctx.lineTo(sx + sw - 16, ly);
      ctx.strokeStyle = `rgba(161,116,10,${p * .25})`;
      ctx.lineWidth = .8;
      ctx.stroke();
    }
    ctx.fillStyle = `rgba(146,64,14,${p * .9})`;
    rrect(ctx, sx - 6, sy - 8, sw + 12, 16, 7);
    ctx.fill();
    rrect(ctx, sx - 6, sy + openH - 8, sw + 12, 16, 7);
    ctx.fill();
    ctx.restore();
    const shimmer = ctx.createRadialGradient(W * .5, H * .5, 0, W * .5, H * .5, 120);
    shimmer.addColorStop(0, `rgba(202,138,4,${p * .15})`);
    shimmer.addColorStop(1, "transparent");
    ctx.fillStyle = shimmer;
    ctx.beginPath();
    ctx.arc(W * .5, H * .5, 120, 0, Math.PI * 2);
    ctx.fill();
  }, 90, onDone);
}
...
function revealAnimSpace(onDone) {
  const stars = Array.from({
    ...
    y: rand(0, 1),
    r: rand(.5, 1.8),
    c: [ "255,255,255", "180,180,255", "255,220,180" ][randI(0, 2)],
    tw: rand(0, Math.PI * 2)
  }));
  revealAnimBase((ctx, W, H, p, t) => {
    ...
    stars.forEach(s => {
      ...
      ctx.beginPath();
      ctx.arc(s.x * W, s.y * H, s.r, 0, Math.PI * 2);
      ctx.fillStyle = `rgba(${s.c},${a * p})`;
      ctx.fill();
    });
    const padY = H * .72;
    ctx.fillStyle = `rgba(100,116,139,${p * .4})`;
    ctx.fillRect(W * .3, padY, W * .4, 8);
    ctx.strokeStyle = `rgba(148,163,184,${p * .4})`;
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.arc(W * .5, padY + 4, 40, 0, Math.PI * 2);
    ctx.stroke();
    ...
    if (t % 2 === 0 && p < .9) {
      ...
      fg.addColorStop(0, "rgba(255,180,40,.8)");
      fg.addColorStop(1, "transparent");
      ctx.beginPath();
      ctx.ellipse(rx, ry + 45, 10, 22 * flame, 0, 0, Math.PI * 2);
      ctx.fillStyle = fg;
      ctx.fill();
    }
    ctx.save();
...
function drawMiniBottle(ctx, x, y) {
  ...
  ctx.lineWidth = .8;
  rrect(ctx, x - 5, y + bob - 11, 10, 17, 3);
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = "rgba(103,196,245,.7)";
  rrect(ctx, x - 3, y + bob - 16, 6, 6, 2);
  ctx.fill();
  ctx.fillStyle = "#c8a96e";
  rrect(ctx, x - 3, y + bob - 19, 6, 4, 2);
  ctx.fill();
  ctx.fillStyle = "rgba(255,252,220,.7)";
  rrect(ctx, x - 3, y + bob - 7, 6, 6, 1);
  ctx.fill();
}

function drawMiniLetter(ctx, x, y, t) {
  const ew = 22, eh = 15, flapAmt = (Math.sin((t || 0) * .08) + 1) / 2;
  ctx.fillStyle = "rgba(224,231,255,.8)";
  ctx.strokeStyle = "rgba(165,180,252,.7)";
  ctx.lineWidth = .7;
  rrect(ctx, x - ew / 2, y - eh / 2, ew, eh, 3);
  ctx.fill();
  ctx.stroke();
  ctx.save();
  ctx.translate(0, y - eh / 2);
  ctx.rotate(-flapAmt * .7);
  ctx.translate(0, -(y - eh / 2));
  ctx.beginPath();
  ctx.moveTo(x - ew / 2, y - eh / 2);
  ctx.lineTo(x, y - eh / 2 + eh * .42);
  ctx.lineTo(x + ew / 2, y - eh / 2);
  ctx.closePath();
  ctx.fillStyle = `rgba(196,181,253,${.5 + flapAmt * .3})`;
  ctx.fill();
  ctx.stroke();
  ctx.restore();
}
...
function drawMiniDove(ctx, x, y, wingPhase) {
  ...
  ctx.fillStyle = "rgba(240,253,244,.8)";
  ctx.strokeStyle = "rgba(187,247,208,.5)";
  ctx.lineWidth = .7;
  ctx.beginPath();
  ctx.ellipse(x - 8, y - 1 + wf, 9, 4, -.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ctx.ellipse(x + 8, y - 1 + wf, 9, 4, .3, 0, Math.PI * 2);
  ...
  ctx.fillStyle = "rgba(240,253,244,.9)";
  ctx.fill();
  ctx.beginPath();
...
function drawMiniLantern(ctx, x, y, glow) {
  ...
  ctx.beginPath();
  ctx.moveTo(x - 7, y - 14);
  ctx.bezierCurveTo(x - 11, y - 5, x - 11, y + 5, x - 7, y + 14);
  ctx.lineTo(x + 7, y + 14);
  ctx.bezierCurveTo(x + 11, y + 5, x + 11, y - 5, x + 7, y - 14);
  ctx.closePath();
  ...
  core.addColorStop(0, `rgba(251,191,36,${glow * .9})`);
  ...
  ctx.beginPath();
  ctx.ellipse(x, y - 14, 4, 2, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(x, y + 14, 4, 2, 0, 0, Math.PI * 2);
  ctx.fill();
...
function drawMiniRocket(ctx, x, y, flame) {
  if (flame > .05) {
    ...
    fl.addColorStop(0, "rgba(255,200,50,.9)");
    fl.addColorStop(1, "transparent");
    ctx.beginPath();
    ctx.ellipse(x, y + 14, 5, 10 * flame, 0, 0, Math.PI * 2);
    ctx.fillStyle = fl;
    ctx.fill();
  }
  ctx.beginPath();
  ...
  ctx.fillStyle = "rgba(186,230,254,.85)";
  ctx.fill();
...
function handleBgHover(mx, my) {
  ...
  if (hit) {
    ...
    if (hoveredObj) {
      hoveredObj = null;
      bgTooltip.classList.remove("show");
...
function drawBg() {
  ...
  base.addColorStop(0, "#06060f");
  base.addColorStop(.5, "#0d0916");
  base.addColorStop(1, "#06060f");
  ...
  bgObjects.forEach(obj => {
    ...
    rrect(bgCtx, obj.x - lw / 2 - 9, obj.y - 44 * obj.scale - 14, lw + 18, 21, 11);
    bgCtx.fill();
    if (isHov) {
      bgCtx.strokeStyle = "rgba(139,124,248,.55)";
      bgCtx.lineWidth = 1;
      rrect(bgCtx, obj.x - lw / 2 - 9, obj.y - 44 * obj.scale - 14, lw + 18, 21, 11);
      bgCtx.stroke();
    }
...
function drawNebula(W, H) {
  [ {
    ...
    y: .7,
    r: .3,
    c: "rgba(192,38,211,.04)"
  }, {
    x: .5,
    y: .5,
    r: .22,
    c: "rgba(14,165,233,.03)"
  } ].forEach(b => {
    ...
    bgCtx.fillStyle = g;
    bgCtx.beginPath();
    bgCtx.arc(W * b.x, H * b.y, W * b.r, 0, Math.PI * 2);
    bgCtx.fill();
  });
}
...
function drawBgBottle(obj) {
  ...
  halo.addColorStop(0, "rgba(14,165,233,.2)");
  halo.addColorStop(1, "transparent");
  ctx.fillStyle = halo;
  ctx.beginPath();
  ctx.arc(0, bob, 36, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(125,211,252,.55)";
  ctx.strokeStyle = "rgba(186,230,253,.7)";
  ctx.lineWidth = 1.2;
  rrect(ctx, -10, bob - 22, 20, 34, 5);
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = "rgba(103,196,245,.6)";
  rrect(ctx, -5.5, bob - 32, 11, 13, 3);
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = "#c8a96e";
  rrect(ctx, -5, bob - 37, 10, 7, 3);
  ctx.fill();
  ctx.fillStyle = "rgba(255,252,220,.7)";
  rrect(ctx, -5.5, bob - 14, 11, 12, 2);
  ctx.fill();
  ...
  for (let w = 0; w < 2; w++) {
    ctx.beginPath();
    for (let px = -24; px <= 24; px += 3) {
      const py = 10 + w * 6 + Math.sin(px / 8 + obj.waveT + w) * 3;
      px === -24 ? ctx.moveTo(px, py) : ctx.lineTo(px, py);
    }
    ctx.stroke();
  }
}
...
function drawBgLetter(obj) {
  ...
  ctx.fill();
  ctx.fillStyle = "rgba(224,231,255,.7)";
  ctx.strokeStyle = "rgba(165,180,252,.6)";
  ctx.lineWidth = 1.2;
  rrect(ctx, -ew / 2, -eh / 2, ew, eh, 4);
  ctx.fill();
  ctx.stroke();
  ...
  ctx.lineTo(0, -eh / 2 + eh * .42);
  ctx.lineTo(ew / 2, -eh / 2);
  ctx.closePath();
  ctx.fillStyle = `rgba(196,181,253,${.5 + fa * .3})`;
  ...
  for (let l = 0; l < 3; l++) {
    ctx.beginPath();
    ctx.moveTo(-ew / 2 + 6, -eh / 2 + 8 + l * 7);
    ctx.lineTo(ew / 2 - 6, -eh / 2 + 8 + l * 7);
    ctx.strokeStyle = "rgba(165,180,252,.3)";
    ctx.lineWidth = .8;
    ctx.stroke();
  }
  ctx.restore();
...
function drawBgDove(obj) {
  ...
  g.addColorStop(0, "rgba(134,239,172,.15)");
  g.addColorStop(1, "transparent");
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.arc(0, 0, 38, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "rgba(240,253,244,.7)";
  ctx.strokeStyle = "rgba(187,247,208,.5)";
  ctx.lineWidth = 1;
  ctx.beginPath();
  ctx.ellipse(-14, -2 + wf, 16, 7, -.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.beginPath();
  ctx.ellipse(14, -2 + wf, 16, 7, .3, 0, Math.PI * 2);
  ctx.fill();
  ctx.stroke();
  ctx.fillStyle = "rgba(240,253,244,.85)";
  ctx.beginPath();
  ctx.ellipse(0, 0, 18, 10, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.arc(18, -4, 7, 0, Math.PI * 2);
  ctx.fill();
  ctx.fillStyle = "#1e293b";
  ctx.beginPath();
...
function drawBgStar(obj) {
  ...
  g.addColorStop(0, `rgba(196,132,252,${glow * .5})`);
  g.addColorStop(.5, `rgba(139,92,246,${glow * .25})`);
  g.addColorStop(1, "transparent");
  ctx.fillStyle = g;
  ctx.beginPath();
  ...
  ctx.fillStyle = `rgba(253,224,71,${.85 + glow * .1})`;
  ctx.shadowColor = "rgba(250,204,21,.8)";
  ctx.shadowBlur = 14;
  ctx.beginPath();
  for (let i = 0; i < 8; i++) {
    const a = i / 8 * Math.PI * 2 - Math.PI / 2, r = i % 2 === 0 ? 18 : 8;
  ...
  ctx.shadowBlur = 0;
  ctx.restore();
  obj.sparkles.forEach(s => {
    s.angle += s.speed;
    ctx.beginPath();
    ctx.arc(Math.cos(s.angle) * s.dist, Math.sin(s.angle) * s.dist, s.size, 0, Math.PI * 2);
    ctx.fillStyle = `rgba(253,224,71,${s.alpha * Math.abs(Math.sin(obj.phase + s.angle))})`;
    ctx.fill();
  });
}
...
function drawBgLantern(obj) {
  ...
  ctx.rotate(sway);
  ctx.translate(0, bob);
  const halo = ctx.createRadialGradient(0, 0, 0, 0, 0, 55);
  halo.addColorStop(0, `rgba(251,146,60,${glow * .35})`);
  halo.addColorStop(1, "transparent");
  ctx.fillStyle = halo;
  ctx.beginPath();
  ...
  ctx.beginPath();
  ctx.moveTo(-14, -28);
  ctx.bezierCurveTo(-22, -10, -22, 10, -14, 28);
  ctx.lineTo(14, 28);
  ctx.bezierCurveTo(22, 10, 22, -10, 14, -28);
  ctx.closePath();
  ctx.fillStyle = "rgba(220,38,38,.8)";
  ctx.strokeStyle = "rgba(153,27,27,.6)";
  ctx.lineWidth = 1.2;
  ctx.fill();
  ctx.stroke();
  const core = ctx.createRadialGradient(0, 0, 0, 0, 0, 16);
  core.addColorStop(0, `rgba(251,191,36,${glow * .9})`);
  core.addColorStop(1, "rgba(239,68,68,.1)");
  ctx.beginPath();
  ...
  ctx.fillStyle = "rgba(251,191,36,.8)";
  ctx.beginPath();
  ctx.ellipse(0, -28, 9, 3.5, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(0, 28, 9, 3.5, 0, 0, Math.PI * 2);
  ctx.fill();
  ctx.strokeStyle = "rgba(153,27,27,.4)";
  ctx.lineWidth = .7;
  ctx.beginPath();
  ctx.moveTo(-14, 0);
  ctx.lineTo(14, 0);
  ...
  obj.particles.forEach(p => {
    ...
    const py = bob - 28 + p.vy * p.life * 80;
    ctx.beginPath();
    ctx.arc(p.x, py, p.size * (1 - p.life * .5), 0, Math.PI * 2);
    ctx.fillStyle = `rgba(251,191,36,${(1 - p.life) * .7})`;
    ctx.fill();
  });
}
...
function drawBgScroll(obj) {
  ...
  const g = ctx.createRadialGradient(0, bob, 0, 0, bob, 42);
  g.addColorStop(0, "rgba(202,138,4,.16)");
  g.addColorStop(1, "transparent");
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.arc(0, bob, 42, 0, Math.PI * 2);
  ctx.fill();
  ctx.save();
  ctx.rotate(tilt);
  ctx.translate(-sw / 2, bob - sh / 2);
  ctx.fillStyle = "rgba(254,249,195,.7)";
  ctx.strokeStyle = "rgba(202,138,4,.5)";
  ctx.lineWidth = 1;
  rrect(ctx, 0, 0, sw, sh, 3);
  ctx.fill();
  ctx.stroke();
  for (let l = 0; l < 3; l++) {
    const ly = 8 + l * 10;
    ...
    ctx.moveTo(5, ly);
    ctx.lineTo(sw - 5, ly);
    ctx.strokeStyle = "rgba(161,116,10,.2)";
    ctx.stroke();
  }
  ctx.fillStyle = "rgba(146,64,14,.8)";
  rrect(ctx, -4, -6, sw + 8, 12, 5);
  ctx.fill();
  rrect(ctx, -4, sh - 6, sw + 8, 12, 5);
  ctx.fill();
  ctx.restore();
}
...
function drawBgSpace(obj) {
  ...
  ctx.rotate(tilt);
  ctx.translate(0, bob);
  const fl = ctx.createRadialGradient(0, 22, 0, 0, 22, 20 * flameAmt);
  fl.addColorStop(0, "rgba(255,200,50,.9)");
  fl.addColorStop(.5, "rgba(255,100,20,.6)");
  fl.addColorStop(1, "transparent");
  ctx.beginPath();
  ctx.ellipse(0, 22, 8, 18 * flameAmt, 0, 0, Math.PI * 2);
  ctx.fillStyle = fl;
  ctx.fill();
  const g = ctx.createRadialGradient(0, 0, 0, 0, 0, 40);
  g.addColorStop(0, "rgba(56,189,248,.12)");
  g.addColorStop(1, "transparent");
  ctx.fillStyle = g;
  ctx.beginPath();
  ctx.arc(0, 0, 40, 0, Math.PI * 2);
  ...
  ctx.fillStyle = "rgba(186,230,254,.8)";
  ...
  obj.particles.forEach(p => {
    ...
    const py = bob + 22 + p.life * 40;
    ctx.beginPath();
    ctx.arc(p.x, py, p.size * (1 - p.life), 0, Math.PI * 2);
    ctx.fillStyle = `rgba(251,146,60,${(1 - p.life) * .5})`;
    ctx.fill();
  });
}
...
async function loadMessages() {
  ...
  const {data: data, error: error} = await supabaseClient.from("messages").select("*").order("created_at", {
    ascending: false
  });
  if (error) {
    console.error(error);
...
function subscribeToMessages() {
  if (!supabaseClient) return;
  supabaseClient.channel("public:messages").on("postgres_changes", {
    event: "INSERT",
    schema: "public",
    table: "messages"
  }, payload => {
    const row = payload.new;
    if (inbox.some(m => m.id === row.id)) return;
